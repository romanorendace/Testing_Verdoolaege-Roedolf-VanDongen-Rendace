...
public class RegisterTest {
	...
	public void setUp() {
		...
		System.setProperty("webdriver.chrome.driver", "C:\\Users\\roman\\OneDrive\\Documents\\Sem3\\Web_3\\chromedriver.exe");
	...
	public void test_Register_AllFieldsFilledInCorrectly_UserIsRegistered() {
		...
		driver.get(path+"?action=users");
		
		ArrayList<WebElement> listItems=(ArrayList<WebElement>) driver.findElements(By.cssSelector("table tr"));
		boolean found = false;
		for (WebElement listItem : listItems) {
				if (listItem.getText().contains("jan.janssens@hotmail.com")
						&&  listItem.getText().contains("Jan")
						&&  listItem.getText().contains("Janssens")) {
				    found = true;
			}
		}
		assertTrue(found);
